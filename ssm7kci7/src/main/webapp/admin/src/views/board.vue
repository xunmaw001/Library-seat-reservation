...
export default {
  ...
  mounted(){
    ...
    this.zuoweixinxiChat2();
    this.getyuyuexuanzuoCount();
    this.yuyuexuanzuoChat3();
  },
  created() {
    this.$nextTick(()=>{
		this.times()
...
//统计接口1
    zuoweixinxiChat2() {
      this.$nextTick(()=>{
        ...
        var zuoweixinxiChart2 = echarts.init(document.getElementById("zuoweixinxiChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                zuoweixinxiChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    zuoweixinxiChart2.resize();
                };
            }
        });
      })
...
//统计接口1
    yuyuexuanzuoChat3() {
      this.$nextTick(()=>{
        ...
        var yuyuexuanzuoChart3 = echarts.init(document.getElementById("yuyuexuanzuoChart3"),'fresh-cut');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                yuyuexuanzuoChart3.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yuyuexuanzuoChart3.resize();
                };
            }
        });
      })
