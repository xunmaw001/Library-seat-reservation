	...
	<view :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
		<form :style='{"width":"calc(100% - 40rpx)","padding":"0","margin":"32rpx 20rpx","background":"none","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"0px 20rpx","margin":"0 0 24rpx 0","borderColor":"#c9f2e4","alignItems":"center","borderRadius":"40rpx","borderWidth":"2rpx","background":"#f3fcf9","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				...
				<input :style='{"border":"0","padding":"0px 24rpx","margin":"0px","color":"#50605d","borderRadius":"8rpx","flex":"1","background":"rgba(255, 255, 255, 0)","fontSize":"28rpx","height":"80rpx"}' v-model="ruleForm.qiandaoshijian" placeholder="签到时间" @tap="toggleTab('qiandaoshijian')"></input>
			</view>
		...
		<w-picker mode="dateTime" step="1" :current="false" :hasSecond="false" @confirm="qiandaoshijianConfirm" ref="qiandaoshijian" themeColor="#333333"></w-picker>
	</view>
</view>
</template>
...
<script>
	...
	export default {
		...
		async onLoad(options) {
            this.ruleForm.qiandaoshijian = this.$utils.getCurDateTime();
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`qiandaoxinxi`, this.ruleForm.id);
		...
		methods: {
			...
			// 日长控件选择日期时间
			qiandaoshijianConfirm(val) {
				console.log(val)
				this.ruleForm.qiandaoshijian = val.result;
				this.$forceUpdate();
			},
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`qiandaoxinxi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`qiandaoxinxi`, this.ruleForm);
						}else{
							await this.$api.add(`qiandaoxinxi`, this.ruleForm);
						}
