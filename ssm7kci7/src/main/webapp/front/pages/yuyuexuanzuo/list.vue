...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
			<view class="cu-bar bg-white search" style="width:100%" :style="[{top:CustomBar + 'px'}]">
				<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
					...
					<view v-if="queryIndex==1" class="search-form round">
						<text class="cuIcon-search"></text>
						<input v-model="searchForm.zuoweibianhao" type="text" placeholder="座位编号" ></input>
					</view>
					<view v-if="queryIndex==2" class="search-form round">
						<text class="cuIcon-search"></text>
						<input v-model="searchForm.zuoweiweizhi" type="text" placeholder="座位位置" ></input>
					</view>
			...
			<view class="uni-product-list" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"border":"2rpx solid #d8f0e7","boxShadow":"0 0px 0px rgba(0,0,0,.3)","margin":"0 0 40rpx","backgroundColor":"#e9f8f1","borderRadius":"16rpx","flexWrap":"wrap","display":"flex","width":"48%","position":"relative","justifyContent":"space-between","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","whiteSpace":"nowrap","overflow":"hidden","color":"#333","width":"70%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis","order":"1"}'>{{product.zuoweibianhao}}</view>
					<image :style='{"padding":"0","margin":"20rpx","objectFit":"cover","borderRadius":"40rpx","display":"block","width":"calc(100% - 40rpx)","height":"300rpx"}' mode="aspectFill" class="uni-product-image" v-if="preHttp(product.zuoweitupian)" :src="product.zuoweitupian.split(',')[0]"></image>
					<image :style='{"padding":"0","margin":"20rpx","objectFit":"cover","borderRadius":"40rpx","display":"block","width":"calc(100% - 40rpx)","height":"300rpx"}' mode="aspectFill" class="uni-product-image" v-else :src="product.zuoweitupian?baseUrl+product.zuoweitupian.split(',')[0]:''"></image>
					<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"0 0 8rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('yuyuexuanzuo','修改')) || (!userid && isAuthFront('yuyuexuanzuo','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('yuyuexuanzuo','删除')) || (!userid && isAuthFront('yuyuexuanzuo','删除'))" @click.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"0","boxShadow":"0 0px 0px rgba(0,0,0,.3)","color":"#666","bottom":"40rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"#c8e3c3","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('yuyuexuanzuo','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"0 0px 0px rgba(0,0,0,.3)","color":"#666","bottom":"40rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"#c8e3c3","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('yuyuexuanzuo','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.zuoweibianhao="";
				this.searchForm.zuoweiweizhi="";
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.zuoweibianhao){
					params['zuoweibianhao'] = '%' + this.searchForm.zuoweibianhao + '%'
				}
				if(this.searchForm.zuoweiweizhi){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`yuyuexuanzuo`, params);
                } else {
                    res = await this.$api.list(`yuyuexuanzuo`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yuyuexuanzuo', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.zuoweibianhao){
					searchForm['zuoweibianhao'] = '%' + this.searchForm.zuoweibianhao + '%'
				}
				if(this.searchForm.zuoweiweizhi){
					searchForm['zuoweiweizhi'] = '%' + this.searchForm.zuoweiweizhi + '%'
				}
				if(this.searchForm.xingming){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`yuyuexuanzuo`, searchForm);
                } else {
                    res = await this.$api.list(`yuyuexuanzuo`, searchForm);
                }
				// 如果是第一页数据置空
